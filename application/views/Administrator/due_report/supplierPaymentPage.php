				...
				<div class="row">
					...
					<div class="col-md-5">
						...
						<div class="form-group">
							...
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="number" class="form-control" v-model="payment.SPayment_amount_cash" required>
							</div>
						</div>
						<div class="form-group">
							...
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="number" class="form-control" v-model="payment.SPayment_amount_bank" required>
							</div>
						</div>
						<div class="form-group" style="display:none;" v-bind:style="{display: payment.SPayment_amount_bank > 0 ? '' : 'none'}">
							<label class="col-md-4 control-label">Bank Account</label>
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<v-select v-bind:options="filteredAccounts" v-model="selectedAccount" label="display_text" placeholder="Select account"></v-select>
				...
				<datatable :columns="columns" :data="payments" :filter-by="filter" style="margin-bottom: 5px;">
					<template scope="{ row }">
						<tr>
							...
							<td>{{ row.SPayment_amount_cash }}</td>
							<td>{{ row.SPayment_amount_bank }}</td>
	...
	new Vue({
		...
		data() {
			return {
				payment: {
					...
					SPayment_amount_cash: 0,
					SPayment_amount_bank: 0,
					SPayment_notes: ''
				},
				payments: [],
				suppliers: [],
				...
				columns: [{
					...
					{
						label: 'Cash Amount',
						field: 'SPayment_amount_cash',
						align: 'center'
					},
					{
						label: 'Bank Amount',
						field: 'SPayment_amount_bank',
						align: 'center'
					},
					{
						label: 'Description',
		...
		methods: {
			...
			saveSupplierPayment() {
				...
				if (this.payment.SPayment_amount_bank > 0 && this.selectedAccount.account_id == '') {
					alert('Select an account');
					return;
				} else {
					this.payment.account_id = this.payment.SPayment_amount_bank > 0 ? this.selectedAccount.account_id : null;
				}
			...
			editPayment(payment) {
				...
				if (payment.SPayment_amount_bank > 0) {
			...
			resetForm() {
				...
				this.payment.SPayment_amount_cash = 0;
				this.payment.SPayment_amount_bank = 0;
