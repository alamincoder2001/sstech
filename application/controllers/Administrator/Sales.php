        ...
        try {
            ...
            foreach ($cart as $cartProduct) {
                ...
                if ($cartProduct->productId == 'P01') {
                    ...
                    $product['Product_Purchase_Rate'] = $cartProduct->salesRate;
                ...
                $productId = $cartProduct->productId == 'P01' ? $ProductId : $cartProduct->productId;
                $purchaseRate = $cartProduct->productId == 'P01' ? $cartProduct->salesRate : $cartProduct->purchaseRate;
                ...
                $inventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ?", [$productId, $this->session->userdata('BRANCHid')])->num_rows();
                if ($inventoryCount == 0) {
                    $inventory = array(
            ...
            // $message = "Dear {$sendToName},\nYour bill is {$currency} {$data->sales->total}. Received {$currency} {$data->sales->cashPaid} + {$data->sales->bankPaid} and current due is {$currency} {$currentDue} for invoice {$invoice}";
